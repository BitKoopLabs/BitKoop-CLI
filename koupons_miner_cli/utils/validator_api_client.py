...
class ValidatorConfig:
    ...
    supervisor_url: str = "http://91.99.203.36/api/"
...
class ValidatorClient:
    ...
    def get_sites_sync(self) -> list[dict[str, Any]]:
        try:
            with create_supervisor_client(
                base_url=self.config.supervisor_url
    ...
    async def _make_validator_request(
        ...
        try:
            ...
            if result.get("success"):
                ...
                # REMOVE THIS PRINT - the base_api_client already logs it
                # print(f"❌ {method} {url} - {error_msg}")
