...
class CouponInfo:
    ...
    last_checked_at: Optional[str] = None  # NEW FIELD ADDED
...
class SupervisorClient:
    ...
    def get_coupons(
        ...
        """
        Get coupons from supervisor API
        ...
        try:
            ...
            # Check if the result is in the new format with data/metadata
            if isinstance(result, dict) and "data" in result:
                # New format - extract just the coupons list
                result = result.get("data", [])

            # Convert raw data to CouponInfo objects
            coupons = []
            for coupon_data in result:
                coupon = CouponInfo(
                    ...
                    last_checked_at=coupon_data.get(
                        "last_checked_at"
                    ),  # NEW FIELD ADDED
                )
                coupons.append(coupon)

            logger.info(f"Retrieved {len(coupons)} coupons from supervisor API")
            return coupons
    ...
    def _get_coupons_with_total_count(
        ...
        # Parse the response data and extract count information
        coupons_data = []
        total_count = 0
        ...
        if isinstance(result, dict):
            ...
            if "data" in result and "metadata" in result:
                ...
                # Structure from your example: {"data": [...], "page": "1", "limit": "20", "total": 5, "hasNextPage": false}
                coupons_data = result.get("data", [])
            ...
            if len(coupons_data) >= limit:
                ...
                try:
                    ...
                    # Try to extract count from response headers or API response
                    if "x-total-count" in self.session.headers:
                        ...
                        # Just estimate there are more by adding some extra
                        # This will at least trigger pagination for the user
    ...
    def get_categories(self, fetch_all: bool = True) -> list[ProductCategoryInfo]:
        ...
        try:
            ...
            # If fetch_all is True, we'll get all pages
            if fetch_all:
                result = self.get_categories_paginated(fetch_all=True)
                return result["categories"]
            else:
                # Just get the first page for backward compatibility
        ...
        while True:
            try:
                ...
                if isinstance(result, dict):
                    ...
                    has_next_page = result.get("hasNextPage", False)
                    logger.debug(
                        f"Parsed paginated response: {len(categories_data)} categories, total: {current_total}, hasNextPage: {has_next_page}"
                    )
                elif isinstance(result, list):
