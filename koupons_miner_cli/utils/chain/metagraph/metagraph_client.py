...
class MetagraphClient:
    ...
    async def refresh_validator_status(
        self, validators: Optional[list[ValidatorInfo]] = None
    ) -> list[ValidatorInfo]:
        """
        Refresh the status of specific validators

        Args:
            validators: Optional list of validators to refresh (default: all cached)

        Returns:
            List of refreshed ValidatorInfo objects
        """
        if validators is None:
            validators = self._validators_cache or []

        if not validators:
            logger.warning("No validators to refresh")
            return []

        logger.info(f"Refreshing status for {len(validators)} validators")
        return await self._validate_validators_batch(validators)
