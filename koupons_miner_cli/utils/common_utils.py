"""
Utility classes for the Koupons CLI - common functionality across commands.
...
from koupons_miner_cli.utils.validator_api_client import create_validator_client
    ...
    def get_site_id(site: str) -> int:
        ...
        try:
            with create_supervisor_client() as supervisor_client:
                ...
                # Normalize the input site for comparison
                ...
                for site_info in sites:
                    ...
                    if (
                        ...
                        logger.info(f"Found site ID {site_info.id} for site: {site}")
                        return site_info.id

                # If exact match not found, log available sites for debugging
                available_domains = [s.domain for s in sites]
                logger.debug(f"Available sites: {available_domains}")

                # Improved error message with suggestion to run list-sites command
                raise ValueError(
                    f"Site '{site}' not found in supervisor. "
                    f"Please run 'koupons list-sites' to see available sites."
                )
...
class BaseValidator:
    ...
    @staticmethod
    def validate_and_get_site_id(
        wallet_manager: WalletManager, site: str, code: str
    ) -> int:
        """
