...
def delete_coupon_code(
    ...
    try:
        ...
        return _execute_deletion(wallet_manager, site, site_id, code, max_validators)

    except UserCancellationError as e:
        return _error_response(wallet_manager, site, code, f"Deletion cancelled: {e}")
    except Exception as e:
        logger.error(f"Error during coupon deletion for {site}: {e}")
        logger.debug(f"Full traceback: {traceback.format_exc()}")
        return _error_response(wallet_manager, site, code, f"Deletion error: {e}")
